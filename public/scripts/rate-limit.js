...
(() => {
	...
	// 访问限制配置（加密）
	const CONFIG = {
		MAX_REQUESTS: 10, // 1分钟内最大请求次数
		WINDOW_SECONDS: 60, // 时间窗口（秒）- 60秒（1分钟）
		BLOCK_SECONDS: 15, // 封锁时间（秒）
		STORAGE_KEY: "r4t3_l1m1t", // 存储键名
		SESSION_KEY: "r4t3_s3ss10n", // 会话标识键名
		// 加密路径
		get BLOCK_PAGE() {
			// 简单的路径加密，防止直接暴露
	...
	// 检测是否是真正的页面刷新（而不是页面内导航）
	const isRealPageLoad = () => {
		try {
			// 方法1: 使用 performance.navigation.type（最可靠）
			// type = 1 表示刷新
			if (performance.navigation && performance.navigation.type === 1) {
				return true;
			}

			// 方法2: 使用 performance API 检查导航类型
			...
			// 方法3: 使用 sessionStorage 跟踪会话
			// 如果 sessionStorage 中没有会话标识，说明是新会话（刷新或新标签页）
			...
			// 如果有会话标识，说明是同一个会话中的页面导航
			// 但我们需要检查是否是真正的刷新
			// 检查 referrer：如果 referrer 为空或与当前页面相同，可能是刷新
			...
			// 3. 刷新页面（某些浏览器）
			if (!referrer) {
				// 但如果有 sessionStorage，说明不是新标签页
				// 可能是页面内导航后的状态
				// 这种情况不应该计入刷新
				return false;
			}

			// referrer 与当前 URL 相同，说明是刷新
	...
	const rateLimit = () => {
		try {
			...
			if (!isRealPageLoad()) {
				...
				if (accessData && isNumber(accessData.blockedUntil) && nowSeconds < accessData.blockedUntil) {
					// 被封锁，重定向到封锁页面
					if (!isBlockPage) {
						safeRedirect(CONFIG.BLOCK_PAGE);
					}
				}
				return; // 不继续执行后续的频率限制逻辑
			}
