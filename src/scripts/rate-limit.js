...
(function() {
    ...
    (function() {
        ...
        function detectConsoleSize() {
            ...
            // 只有当差异超过20%时才认为控制台打开
            // 这样可以避免因浏览器窗口大小调整或其他因素导致的误触发
            return (
                widthDiffRatio > 0.2 || heightDiffRatio > 0.2
            );
        }
        
        // 综合检测
        window.detectDevTools = function() {
            return checkDevTools() || detectDebugger() || detectConsoleSize();
        };
    })();
    ...
    setInterval(() => {
        ...
        const isBlocked = isNumber(accessData?.blockedUntil) && nowSeconds < accessData.blockedUntil;
